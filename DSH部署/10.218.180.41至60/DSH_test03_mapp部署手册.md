...
> 配套材料：本目录 `files/`（`test03.env`、`dsh-web-test03.service` 成稿）；nginx 侧成稿 `nginx-1.30.5/DSH配置/simbest.conf`
...
| `test03.env`、`dsh-web-test03.service` 成稿 | 本目录 `files/`，与第 4、6 节内容一致 |
...
# 方式 B：上传成稿 files/test03.env
# scp files/test03.env mapp@10.218.180.45:/data/dsh/dsh-etc/instances/test03.env
```

文件内容（成稿 `files/test03.env` 与此一致；需要 LLM 时填入真实 key）：
...
# 方式 B：上传成稿 files/dsh-web-test03.service
# scp files/dsh-web-test03.service mapp@10.218.180.45:/data/dsh/.config/systemd/user/dsh-web-test03.service
```

unit 全文（成稿 `files/dsh-web-test03.service` 与此一致）：
