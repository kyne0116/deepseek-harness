...
**B 架构内存档位 → 安全人数速查**（上面 B 公式的反解：`安全人数 ≤ (总内存 × 0.7 − 2GB) ÷ 300MB`）：

| 单机内存 | 公式乐观上限（300MB/人） | **推荐安全线（按上方分方式配置档位折算 ≈ 450MB/人）** | 配套 CPU |
| --- | --- | --- | --- |
| 32 GB | ~68 人 | **50 人** | 8C |
| 64 GB | ~142 人 | **100 人** | 16C |
| 128 GB | ~292 人 | 200 人应**拆 2×(16C/64GB)** 分片，单机不建议硬扛 | — |
| 256 GB | ~592 人 | 按分片拆机，单机不建议超 100~150 常驻实例 | — |

> 安全线比公式上限再保守约 50%，余量用于吸收长会话/图片压缩的 **500MB+ 峰值毛刺**与页缓存波动；
> 落地后以实测校准：`ps -u <账号> -o rss= | awk '{s+=$1} END {printf "%.1f GB\n", s/1024/1024}'`（全实例 RSS 汇总）、
> `systemctl --user show dsh-web-45@<实例> -p MemoryCurrent`（单实例 cgroup 口径）。
